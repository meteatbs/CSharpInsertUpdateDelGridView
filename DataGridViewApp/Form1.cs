...
namespace DataGridViewApp
{
    ...
    {
        string connectionString = @"Data Source=DESKTOP-84LNPPD;Initial Catalog=DGVDB;Integrated Security=True";
        public Form1()
        {
            InitializeComponent();
        }
        ...
        private void dgvEmployee_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Are you sure to delete this data?", "DataGridView", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    using (SqlConnection sqlCon = new SqlConnection(connectionString))
                    {
                        sqlCon.Open();
